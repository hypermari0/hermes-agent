...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "thread_id": thread_id,
            # sender_id: consumed by cron/scheduler.py to run Composio under the
            # right entity (fork feature). user_id: captured so an opt-in delivery
            # mirror (cron.mirror_delivery / attach_to_session) can resolve the
            # exact participant's session in per-user-isolated group chats —
            # parity with interactive send_message, which passes
            # HERMES_SESSION_USER_ID to gateway.mirror.mirror_to_session.
            # Both derive from HERMES_SESSION_USER_ID.
